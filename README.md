...
GitHub Actions 的标准 GitHub-hosted runner 在公开仓库中免费使用；私有仓库会按账号或组织套餐消耗每月免费分钟数。官方文档见 [GitHub Actions billing](https://docs.github.com/en/billing/concepts/product-billing/github-actions)。
...
本项目默认使用 `ubuntu-latest` 标准 runner。一次同步通常只运行几十秒到几分钟；如果按默认每 3 天运行一次，正常情况下很难用完个人免费额度。
